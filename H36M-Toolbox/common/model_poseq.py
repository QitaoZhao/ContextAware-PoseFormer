...
class PoseTransformer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,  norm_layer=None, embed_3d=False, num_bins=1000):
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(embed_dim, num_joints*sum(num_bins)),
            # nn.GELU(),
            # nn.Linear(embed_dim, embed_dim),
            # nn.LayerNorm(embed_dim_ratio),
        )

        # self.output_x = nn.Linear(embed_dim, num_bins[0])
        # self.output_y = nn.Linear(embed_dim, num_bins[1])
        # self.output_z = nn.Linear(embed_dim, num_bins[2])
    ...
    def Spatial_forward_features(self, x):
        ...
        # if self.embed_3d:
        #     cam = cam.repeat_interleave(f, dim=0).view(-1, 3, 3)
        #     x += self.proj_3d((cam @ self.Spatial_pos_embed_3d).view(b*f, -1)).view(b*f, 1, -1)
    ...
    def forward(self, x, cam=None): # [256, f, 17, 2]
        ...
        if self.embed_3d:
            # cam = self.camera_intrinsics.repeat(b, 1)
        ...
        x = self.forward_features(x) # [256, 1, 17, 32]
        x = self.head(x).view(b, 1, p, -1) # [256, 1, 17, 544]

        # return [self.output_x(x), self.output_y(x), self.output_z(x)]
        return [x[:, :, :, :self.num_bins[0]], x[:, :, :, self.num_bins[0]:self.num_bins[0]+self.num_bins[1]], x[:, :, :, -self.num_bins[2]:]]
