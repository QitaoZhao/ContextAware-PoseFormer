...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, poses_2d_crop, valid_frame,
        ...
        self.train_img_path = "dataset/mpi_inf_3dhp/images"
        self.test_img_path = "dataset/mpi_inf_3dhp_test_set/images"
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        if self.train == True:
            ...
            subdir = subdir_format.format(int(subject[1]), int(seq[3]), int(cam_index))
        ...
        fileformat = img_dir + '/' + subdir + '_{:06d}.jpg'
        file_path = fileformat.format(start_3d + 1)
        ...
        if self.train == True:
            ...
            if self.poses_3d is None and self.cameras is None:
                ...
                return np.zeros(9), self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), seq, subject, int(cam_index)
            elif self.poses_3d is None:
                return self.batch_cam, None, self.batch_2d.copy(),seq, subject, int(cam_index)
            else:
                return self.batch_cam, self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), seq, subject, int(cam_index)
        else:
            if self.MAE:
                return np.zeros(9), self.batch_2d.copy(), seq_name, None, None
            else:
                return np.zeros(9), self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), seq_name, None, None
