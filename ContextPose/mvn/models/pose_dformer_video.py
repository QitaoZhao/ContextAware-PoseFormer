...
class PoseTransformer_video(nn.Module):
    def __init__(self, config=None, num_frame=9, num_joints=17, in_chans=2,
        ...
        out_dim = 3 * num_joints    #### output dimension is num_joints * 3
        ...
        self.temproal_blocks = nn.ModuleList([
            Block(
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(num_joints * embed_dim_ratio, embed_dim_ratio),
    ...
    def forward(self, keypoints_2d, ref, features_list):
        ...
        bf = b*f
        ### keypoints_2d:  [batch_size * receptive frames, joint_num, 2 channels],
        keypoints_2d = keypoints_2d.reshape(bf, p, c)
        x = self.coord_embed(keypoints_2d)  ## [b * f, j, embed_dim_ratio],

        features_ref_list = [
            F.grid_sample(features, keypoints_2d.unsqueeze(-2), align_corners=True).squeeze(-1).permute(0, 2, 1).contiguous() \
            for features in features_list]

        features_ref_list = [embed(features_ref_list[idx]) \
                             for idx, embed in enumerate(self.feat_embed)]

        x = torch.stack([x,*features_ref_list], dim=1) # [b*f, 5, p, c]


        x += self.Spatial_pos_embed
        x = self.pos_drop(x)

        x = x.reshape(-1,  p, self.embed_dim_ratio)

        for blk in self.joint_blocks:
            x = blk(x)

        x = x.reshape(bf, -1, p*self.embed_dim_ratio)
        x = self.fc1(x)  ### x: [bf, l, dim_j]
        ...
        for blk in self.temproal_blocks:
            x = blk(x)
