...
from mvn.utils.op import integrate_tensor_3d_with_coordinates
...
COLOR_DICT = {
    ...
    'human36m': [
        (0,139,139), (0,139,139), (0,139,139),  # right leg (green)
        (255,99,71), (255,99,71), (255,99,71),  # left leg
        (255,215,0), (255,215,0),  # body
        (255,215,0), (255,215,0),  # head
        (0,139,139), (0,139,139), (0,139,139),   # right arm
        (255,99,71), (255,99,71), (255,99,71)   # left arm
    ],
...
def visualize_batch(images_batch, heatmaps_batch, keypoints_2d_batch, proj_matricies_batch,
    ...
    fig, axes = plt.subplots(ncols=n_cols, nrows=n_rows, figsize=(n_cols * size+10, n_rows * size))
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            if col == 0:
                ...
                heatmap = cv2.applyColorMap((heatmaps[row, col - 1] * 255).clip(0, 255).astype(np.uint8), cv2.COLORMAP_JET)
                axes[row, col].imshow(cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB), alpha=0.5)
...
def visualize_volumes(images_batch, volumes_batch, proj_matricies_batch,
                      ...
                      batch_index=0, size=5,
                      max_n_rows=10, max_n_cols=18):
    # volumes_batch (b, 17, 64, 64, 64)
    batch_size, n_joints = volumes_batch.shape[0], volumes_batch.shape[1]

    n_cols, n_rows = min(n_joints + 1, max_n_cols), min(batch_size, max_n_rows)
...
def draw_2d_pose(keypoints, ax, kind='cmu', keypoints_mask=None, point_size=2, line_width=1, radius=None):
    """
    Visualizes a 2d skeleton

    Args
        keypoints numpy array of shape (J, 2): pose to draw in CMU format.
    ...
    for i, joint in enumerate(connectivity):
        if keypoints_mask[joint[0]] and keypoints_mask[joint[1]]:
            xs, ys = [np.array([keypoints[joint[0], j], keypoints[joint[1], j]]) for j in range(2)]
            ...
            ax.plot(xs, ys, c=color, lw=4*line_width, zorder=set_zorder, solid_capstyle='round')

    ax.scatter(keypoints[:, 0], keypoints[:, 1], s=5*point_size, c='black', edgecolors='black', zorder=3)  

    if radius is not None:
        root_keypoint_index = 6
