...
from mvn.models.conpose_video import VolumetricTriangulationNet
...
def setup_human36m_dataloaders(config, is_train, distributed_train, rank = None, world_size = None):
	...
	##### add ############################
	val_sampler = torch.utils.data.distributed.DistributedSampler(val_dataset, num_replicas=world_size, rank=rank) if distributed_train else None
	##### add ############################
		...
		while batch is not None:
			...
			if model_type == "vol":
				...
				if (not is_train) and config.val.flip_test:
					# print("images_batch[:, 0]", images_batch[:, 0].shape)
...
def main(args):
	...
	}[config.model.name](config, args.frame, device)
	...
	if args.eval:
		checkpoint = torch.load("checkpoint/best_epoch_video.bin")['model']
		...
		print("Loading checkpoint from {}".format("checkpoint/best_epoch_video.bin"))
	...
	if not args.eval:
		...
		for epoch in range(config.train.n_epochs):
			...
			if config.model.name == 'vol':
				...
				if master:
					for k in result.keys():
						...
						# errors_p3.append(result[k]['N_MPJPE'] * 1000)ss

					error_p1 = round(np.mean(errors_p1), 2)
					error_p2 = round(np.mean(errors_p2), 2)
			...
			if master:
				...
				if (epoch+1)%5 == 0:
					file_name = f'checkpoints/checkpoint{epoch}.bin'
					save_path = os.path.join(experiment_dir, file_name)
					torch.save({
						'epoch': epoch + 1,
						'lr': lr,
				...
				if error_p1 < min_loss:
					...
					torch.save({
						...
						'optimizer': optimizer.state_dict(),
						'model': model.state_dict(),
					}, os.path.join(experiment_dir, "checkpoints/best_epoch_video.bin"))
