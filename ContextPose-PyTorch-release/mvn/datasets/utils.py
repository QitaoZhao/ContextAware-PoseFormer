...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            # images_batch_copy = self.next_batch[0]
            for i in range(len(self.next_batch)):
                self.next_batch[i] = self.next_batch[i].cuda(non_blocking=True).to(self.device)
                # self.next_batch[i] = self.next_batch[i].float().cuda(non_blocking=True).to(self.device)

            images_batch, keypoints_3d_gt, keypoints_2d_batch_cpn, keypoints_2d_batch_cpn_crop = self.next_batch
            # images_batch, keypoints_3d_gt, keypoints_2d_batch_cpn, keypoints_2d_batch_cpn_crop, keypoints_2d_batch_gt_crop = self.next_batch
            # keypoints_3d_gt, keypoints_2d_batch_cpn, keypoints_2d_batch_cpn_crop = self.next_batch
