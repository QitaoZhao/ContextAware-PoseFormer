...
class FuseBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_levels=4, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlps = nn.ModuleList([Mlp(in_features=dim // 5, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop) for i in range(num_levels+1)])
        # self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x):
        b, p, c = x.shape
        residual = x + self.drop_path(self.attn(self.norm1(x)))
        x = self.norm2(residual)
        x = torch.split(x, c//5, dim=2)
        x = [mlp(x[idx]) for idx,mlp in enumerate(self.mlps)]
        x = torch.cat(x, dim=2)
        x = residual + self.drop_path(x)

        # x = self.norm2(residual)
        # x_0, x_1 = x[:, :1], x[:, 1:]
        # x = torch.cat([self.mlp_0(x_0), self.mlp_1(x_1)], dim=1)
...
class PoseTransformer(nn.Module):
    def __init__(self, config, num_frame=1, num_joints=17, in_chans=2,
        ...
        self.Spatial_pos_embed = nn.Parameter(torch.zeros(1, 1, num_joints, embed_dim_ratio))
    ...
    def forward(self, keypoints_2d, ref, features_list):
        ...
        x = self.coord_embed(keypoints_2d).view(b, 1, p, -1)

        # features_ref_list = [
        #     F.grid_sample(features, ref.unsqueeze(-2), align_corners=True).squeeze().permute(0, 2, 1).contiguous() \
